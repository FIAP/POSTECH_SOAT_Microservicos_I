from sqlalchemy import create_engine, Table, MetaData, select, update, insert, Column, String, Text, Integer, Float, ForeignKey, and_, or_, not_
    ...
    def atualizar_item_kit(self, item_kit: ItemKit, on_not_found: Exception, on_outdated_version: Exception):
        ...
        try:
            ...
            update_query_item_kit = update(self.__item_kit_table).where(
                    self.__item_kit_table.c.id == item_kit.id, 
                    self.__item_kit_table.c.versao == versao
                ).values(
                    versao=nova_versao,
                    qtd=item_kit.qtd
                )
